...
public class LogController {
    ...
    @GetMapping("userInitPage")
    public String userInitPage() {
        Page<LogRecord> page = logService.page(new Page<LogRecord>(1, 100));
        return new JSONObject(page).toString();
    }
    ...
    @PostMapping("userQueryTime")
    public String userQueryTime(@RequestBody JSONObject times) {
        return new JSONObject(logService.page(new Page<LogRecord>(times.getInt("current"), times.getInt("size")),
                new QueryWrapper<LogRecord>().between("begintime", times.getStr("begin"), times.getStr("end")))).toString();
    }
}
